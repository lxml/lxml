        ...
        while c_nsdef is not NULL:
            if c_ns_list_len >= c_ns_list_size:
                if c_ns_list is NULL:
                    ...
                    c_ns_list_size *= 2
                c_nsref_ptr = <_ns_node_ref*> realloc(
...
cdef xmlNs* _searchNsByHref(xmlNode* c_node, char* c_href, bint is_attribute):
    ...
    if c_href is NULL or c_node is NULL or c_node.type == tree.XML_ENTITY_REF_NODE:
        return NULL
    if strcmp(c_href, tree.XML_XML_NAMESPACE) == 0:
    ...
    while c_node is not NULL:
        if c_node.type == tree.XML_ELEMENT_NODE:
            ...
            while c_ns is not NULL:
                if c_ns.href is not NULL and strcmp(c_href, c_ns.href) == 0:
    ...
    if c_name is NULL:
        if c_href is NULL:
            ...
            if c_node_href is NULL:
                ...
                return strcmp(c_node_href, c_href) == 0
    elif c_href is NULL:
        if _getNs(c_node) is not NULL:
            return 0
        return c_node.name == c_name or strcmp(c_node.name, c_name) == 0
    elif c_node.name == c_name or strcmp(c_node.name, c_name) == 0:
        c_node_href = _getNs(c_node)
        if c_node_href is NULL:
            return c_href[0] == c'\0'
        else:
...
cdef inline bint _tagMatchesExactly(xmlNode* c_node, char* c_href, char* c_name):
    ...
    * its name string points to the same address (!) as c_name
    ...
    if c_href is NULL:
        ...
        return strcmp(c_href, c_node_href) == 0
...
cdef object _decodeFilename(char* c_path):
    ...
    """
    cdef Py_ssize_t c_len = strlen(c_path)
