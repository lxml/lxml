...
COMMENT = f"""
# This section is generated by the script '{os.path.basename(sys.argv[0])}'.
...
def main(doc_dir):
    ...
    for enum_name in ENUM_ORDER:
        ...
        append_pxd(ctypedef_indent + f'ctypedef enum {enum_name}:')
        append_pxi(f'cdef object {pxi_name} = ""\"\\')
    ...
    # write source files
    print(f"Updating file {BUILD_SOURCE_FILE}")
    updated = regenerate_file(BUILD_SOURCE_FILE, pxi_result)
    if not updated:
        print("No changes.")

    print(f"Updating file {BUILD_DEF_FILE}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or sys.argv[1].lower() in ('-h', '--help'):
        print(f"This script generates the constants in file {BUILD_SOURCE_FILE}")
        print("Call as")
